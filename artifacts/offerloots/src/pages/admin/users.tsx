...
import { Search, MoreVertical, Eye, ShieldBan, ShieldCheck, DollarSign, UserPlus } from "lucide-react";
...
export default function AdminUsers() {
  ...
  const isMainAdmin = currentUser?.id === MAIN_ADMIN_ID;
  ...
  const handleCreateAdmin = async () => {
    ...
    setCreating(true);
    try {
      await customFetch("/api/admin/create-admin", {
        method: "POST",
        body: JSON.stringify(createForm),
      });
      toast({ title: "Admin created", description: `${createForm.username} is now an admin.` });
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <Button onClick={() => setCreateAdminOpen(true)} className="gap-2">
                ...
                <tbody>
                  {isLoading ? (
                    ...
                    usersData?.data.map((user) => {
                      const isProtected = user.id === MAIN_ADMIN_ID && !isMainAdmin;
                      return (
                        <tr key={user.id} className="border-b last:border-0 hover:bg-muted/20">
                          ...
                          <td className="px-6 py-4">
                            ...
                            {user.id === MAIN_ADMIN_ID && <Badge variant="outline" className="ml-2 border-yellow-500 text-yellow-500">Main</Badge>}
                          </td>
